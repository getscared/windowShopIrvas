...
window.addEventListener('DOMContentLoaded', function () {
  'use strict'; //Обьект для хранения ответов пользователя в модальных окнах changeModalState.js

  var modalState = {
    form: 0,
    type: 'tree'
  };
  var deadline = '2023-06-20'; //Вызов функции выбора окна с пустым обьектом в атрибутах
...
var calcScroll = function calcScroll() {
  ...
  document.body.appendChild(div); //Вычисляем ширину посолы прокрутки
...
var changeModalState = function changeModalState(state) {
  ...
  var btnParrent = document.querySelector('.popup_calc_content');
  var btn = btnParrent.querySelector('.popup_calc_button');
  Object(_checkNumInputs__WEBPACK_IMPORTED_MODULE_1__["default"])('#width');
  Object(_checkNumInputs__WEBPACK_IMPORTED_MODULE_1__["default"])('#height'); //перебор окон и запись выбранного варианта в обьект modalState in main.js
...
var forms = function forms(state) {
  ...
  var inputs = document.querySelectorAll('input'); //Для очищения инпутов после отправки данных на сервер
    ...
    return regeneratorRuntime.async(function postData$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            //async- внутри функции есть асинхронные операции
            //Вывод сообщения о начале отправки
            //Находим блок созданный для вывода сообщений
            document.querySelector('.status').textContent = message.loading; //Отправка запроса

            _context.next = 3;
            return regeneratorRuntime.awrap(fetch(url, {
              //await перед асинхронной операцией. Ждет ответа на запрос
              method: "POST",
              body: data
            }));
  ...
  form.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      ...
      var formData = new FormData(item); //Конструктор FormData соберет все данные из выбранной формы
...
var images = function images() {
  ...
  workSection.addEventListener('click', function (e) {
    ...
    if (target && target.classList.contains('preview')) {
      ...
      document.body.style.marginRight = "".concat(scroll, "px"); //получаем ссылку на большое изображение

      var path = target.parentNode.getAttribute('href'); //передаем полученную ссылку в bigImg
    ...
    if (target && target.matches('div.popup')) {
      //если target совдает с div с классом .popup
      imgPopup.style.display = 'none';
...
var tabs = function tabs(headerSelector, tabSelector, contentSelector, activeClass) {
  ...
  function showTabContent() {
    ...
    content[i].style.display = display; //add active class to tab

    tab[i].classList.add(activeClass);
  }

  hideTabContent();
  showTabContent(); //Навешиваем обработчик событий на общую область которая объеденяет все табы.

  header.addEventListener('click', function (e) {
    var target = e.target; //Проверяем что кликнули в один из табов.
...
var timer = function timer(id, deadline) {
  ...
  var setTime = function setTime(selector, endtime) {
    ...
    var timeInterval = setInterval(updateClock, 1000); // Функция обновления счетчика
